...
const staticGameData = {
  ...
  "oppCards": ["ZS1", "HP1", "HP2", "AP1", "AP2", "AT1", "AT2", "HD1", "HD3", "PS1", "PS2"],
...
io.on('connection', (socket) => {
  ...
  socket.on("playCard", (data) => {
    ...
    if (!gameSession || gameSession.startState !== 1) {
      ...
      } else if (oppInd !== -1 && staticGameData.rivalCards[data.cardId.substring(0, 2)].includes(gameSessions[gameSessionInd].players[oppInd].selectedGod)) {
        stabModifier = 0.75;
      ...
      if (oppInd !== -1 && gameSessions[gameSessionInd].players[oppInd].invulnerable !== 0) {
        ...
        switch (data.cardId) {
          ...
          case "HP1":
            ...
            break;
          case "HP2":
            gameSessions[gameSessionInd].players[playerInd].invulnerable = 3;
            break;
          case "HP3":
          ...
          case "PS2": // to be resolved
            damage = getDamage(90/(gameSessions[gameSessionInd].players.length - 1), 100, stabModifier, gameSessions[gameSessionInd].players[playerInd].strength, gameSessions[gameSessionInd].players[oppInd].defense, gameSessions[gameSessionInd].players[playerInd].affinity, gameSessions[gameSessionInd].players[oppInd].affinity);
            oppHealthChange = -damage;
            gameSessions[gameSessionInd].players[oppInd].burn = 0;
            gameSessions[gameSessionInd].players[oppInd].invulnerable = 0;
            break;
          case "PS3":
      ...
      if (gameSessions[gameSessionInd].players[playerInd].burn > 0) {
        gameSessions[gameSessionInd].players[playerInd].burn -= 1;
        selfHealthChange -= 10;
      }
      // Invulnerability
      ...
      if (oppInd !== -1) {
        ...
        gameSessions[gameSessionInd].players[oppInd].defense += oppDefenseChange;
